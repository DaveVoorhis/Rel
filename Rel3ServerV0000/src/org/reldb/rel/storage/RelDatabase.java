...
import org.reldb.rel.languages.tutoriald.parser.*;
import org.reldb.rel.storage.catalog.*;
...
import org.reldb.rel.storage.relvars.RelvarPrivateCell;
import org.reldb.rel.storage.relvars.RelvarReal;
import org.reldb.rel.storage.relvars.RelvarRealMetadata;
import org.reldb.rel.storage.tables.KeyTableNames;
import org.reldb.rel.storage.tables.KeyTables;
import org.reldb.rel.storage.tables.RegisteredTupleIterator;
import org.reldb.rel.storage.tables.Table;
import org.reldb.rel.storage.tables.TablePrivate;
import org.reldb.rel.types.TypeAlpha;
import org.reldb.rel.types.builtin.*;
...
public class RelDatabase {
	...
	private HashMap<String, org.reldb.rel.types.Type> typeCache = new HashMap<String, org.reldb.rel.types.Type>();
    ...
    public synchronized org.reldb.rel.types.Type loadType(Generator generator, String typeName) {
    	...
    	org.reldb.rel.types.Type type = typeCache.get(typeName);
    	if (type == null) {
	    	try {
		    	...
		    	if (language.equals("Rel")) {
					...
					try {
			            ...
			            Constructor<?> ctor = typeClass.getConstructor(new Class[] {org.reldb.rel.generator.Generator.class});
			            if (ctor == null)
			            	throw new ExceptionSemantic("RS0211: Unable to find a constructor of the form " + typeName + "(Generator generator)");
						type = (org.reldb.rel.types.Type)(ctor.newInstance(generator));
    ...
    public synchronized void dropType(final Generator generator, final String typeName) {
    	final org.reldb.rel.types.Type type = loadType(generator, typeName);
    	try {
	    	(new TransactionRunner() {
	    		public Object run(Transaction txn) throws Throwable {
			...
			while (implementationIterator.hasNext()) {
				...
				if (existingImplementationSignature.equalsIgnoreCase(signature.toRelLookupString())) {
			    	...
			    	if (language.equals("Rel") || language.equals("System")) {
				    	...
				    	org.reldb.rel.types.Type returnType;
						try {
