...
VAR PointsForSolving VIRTUAL summarize ( join { SolvedBy , Problem , SP } ) by { FirstName , LastName } add ( SUM ( DR ) AS MoreMPs , SUM ( SPs ) AS MoreSPs );

ANNOUNCE 'var PointsToBeAdded';
VAR PointsToBeAdded VIRTUAL extend ( join { ( extend SolvedBy add ( 'S' as SorC ) union extend ComposedBy add ( 'C' as SorC ) ) , Problem , SP } ) add ( ( CASE when SorC = 'C' then ( DR + 3 ) / 4 else SPs end case ) as ActSPs );

ANNOUNCE 'var AllTimeStep1';
VAR AllTimeStep1 VIRTUAL ( extend ( join { Problemist , summarize PointsToBeAdded by { FirstName , LastName } add ( SUM ( DR ) AS MoreMPs , SUM ( ActSPs ) AS MoreSPs ) } ) add ( MPs + MoreMPs AS TotalMPs , SPs + MoreSPs as TotalSPs ) ) { FirstName , LastName , TotalMPs , TotalSPs } rename ( TotalMPs AS MPs , TotalSPs as SPs ) union ( Problemist not matching SolvedBy );

ANNOUNCE 'var AllTimeGraded';
VAR AllTimeGraded VIRTUAL ( extend AllTimeStep1 add ( Grade from tuple from ( join { Grade , summarize ( Grade where ReqMPs <= MPs and ReqSPs <= SPs ) ADD ( MAX ( ReqMPs ) AS ReqMPs ) } ) as Grade ) );

ANNOUNCE 'var AllTimeForWeb';
VAR AllTimeForWeb VIRTUAL extend AllTimeGraded add ( FirstName || ' ' || LastName as Name ) { ALL BUT FirstName , LastName };

ANNOUNCE 'var League';
VAR League VIRTUAL summarize ( ( PointsToBeAdded where Year = MAX ( Problem , Year ) ) { ALL BUT SorC , Month , Year , NumberOfComposers , SPs } ) by { FirstName , LastName } add ( SUM ( DR ) AS MPs , SUM ( ActSPs ) AS SPs );

ANNOUNCE 'var PointsExcludingLatest';
VAR PointsExcludingLatest VIRTUAL with MAX ( Problem , Problem# ) AS Latest : extend ( join { ( extend SolvedBy where Problem# < Latest add ( 'S' as SorC ) union extend ComposedBy where Problem# < Latest add ( 'C' as SorC ) ) , Problem , SP } ) add ( ( CASE when SorC = 'C' then ( DR + 3 ) / 4 else SPs end case ) as ActSPs );

ANNOUNCE 'var GradeBeforeLatest';
VAR GradeBeforeLatest VIRTUAL ( extend ( join { Problemist , summarize PointsExcludingLatest by { FirstName , LastName } add ( SUM ( DR ) AS MoreMPs , SUM ( ActSPs ) AS MoreSPs ) } ) add ( MPs + MoreMPs AS TotalMPs , SPs + MoreSPs as TotalSPs , Grade from tuple from ( join { Grade , summarize ( Grade where ReqMPs <= TotalMPs and ReqSPs <= TotalSPs ) ADD ( MAX ( ReqMPs ) AS ReqMPs ) } ) as PrevGrade ) ) { FirstName , LastName , TotalMPs , TotalSPs , PrevGrade } rename ( TotalMPs AS MPs , TotalSPs as SPs );

ANNOUNCE 'var NewTotals';
VAR NewTotals VIRTUAL with MAX ( SolvedBy , Problem# ) AS Latest : ( extend ( join { Problemist , summarize ( PointsToBeAdded matching ( ( ( SolvedBy where Problem# = Latest ) union ( ComposedBy where Problem# = Latest ) ) { FirstName , LastName } ) ) by { FirstName , LastName } add ( SUM ( DR ) AS MoreMPs , SUM ( ActSPs ) AS MoreSPs ) } ) add ( MPs + MoreMPs AS TotalMPs , SPs + MoreSPs as TotalSPs ) ) { FirstName , LastName , TotalMPs , TotalSPs } rename ( TotalMPs AS MPs , TotalSPs as SPs );

ANNOUNCE 'var NewGrades';
VAR NewGrades VIRTUAL ( extend NewTotals add ( Grade from tuple from ( join { Grade , summarize ( Grade where ReqMPs <= MPs and ReqSPs <= SPs ) ADD ( MAX ( ReqMPs ) AS ReqMPs ) } ) as Grade ) );
...
CONSTRAINT RightNumberOfDTYcomposers IS_EMPTY ( DTYprob not matching summarize DTYcomposer per ( DTYprob { DTY# } ) add ( COUNT ( ) AS NumberOfComposers ) );
