...
public class Generator {
	...
	// When set, collects references to global relvars, operators, and user-defined
	// types.
	// Used to prevent DROPping VIRTUAL relvars, operators, constraints, and types
	// that use these.
	...
	/**
	 * Turn code generation on or off. Useful for doing type checking without
	 * generating code.
	 */
	...
	private DebugInfo getDebugInfo() {
		if (parser != null)
			return new DebugInfo(parser.getCurrentNode(),
					getOperatorDefinitionLineReferenceStack(parser.getCurrentNode().first_token.beginLine));
	...
	public void defineRelvarReal(String varname, RelvarHeading keydef, References references) {
		if (currentOperatorDefinition.getDepth() > interactiveOperatorNestingDepth)
			throw new ExceptionSemantic(
					"RS0018: REAL relation-valued variables may not be defined inside a user-defined operator.");
		...
		RelvarDefinition relvar = new RelvarDefinition(varname,
				new RelvarRealMetadata(database, keydef, userRelvarOwner), references);
	...
	public void defineRelvarPublic(String varname, RelvarHeading expectedKeydef) {
		...
		// TODO - type check can be less restrictive; if heading of actualRelvarType is
		// a subset of expectedType it should be acceptable
		if (!expectedKeydef.getHeading().canAccept(actualRelvarType.getHeading()))
			throw new ExceptionSemantic("RS0021: Expected relation-valued variable '" + varname + "' to be "
					+ new TypeRelation(expectedKeydef.getHeading()) + " but got " + actualRelvarType);
		// TODO - check that expectedKeydef is a subkey of metadata.getKeyDefinition()
		// here...
	...
	public void defineRelvarVirtual(String varname, String sourceCode, RelvarHeading keydef, References references) {
		if (currentOperatorDefinition.getDepth() > interactiveOperatorNestingDepth)
			throw new ExceptionSemantic(
					"RS0021: VIRTUAL relation-valued variables may not be defined inside a user-defined operator.");
		if (relvarsInProgress.containsKey(varname) || database.isRelvarExists(varname))
			throw new ExceptionSemantic("RS0022: " + varname + " already exists.");
		RelvarDefinition relvar = new RelvarDefinition(varname,
				new RelvarVirtualMetadata(database, sourceCode, keydef, userRelvarOwner), references);
		relvarsInProgress.put(varname, relvar);
		beginAssignment();
	...
	public void defineRelvarExternal(String varname, String externalRelvarType, String externalRelvarSpecification,
			String duplicates) {
		...
		if (currentOperatorDefinition.getDepth() > interactiveOperatorNestingDepth)
			throw new ExceptionSemantic(
					"RS0024: EXTERNAL relation-valued variables may not be defined inside a user-defined operator.");
		if (relvarsInProgress.containsKey(varname) || database.isRelvarExists(varname))
		...
		try {
			...
			throw new ExceptionSemantic(
					"RS0451: Can't find " + className + " to handle external relvar of type " + externalRelvarType);
		}

		RelvarCustomMetadata metadata = null;
		try {
			metadata = (RelvarCustomMetadata) clazz.getConstructors()[0].newInstance(database, userRelvarOwner,
					externalRelvarSpecification, handler);
	...
	public Value getTypeOf(Heading heading, String headingIn) {
		...
		for (Attribute attribute : heading.getAttributes()) {
			Value[] values = new Value[] { ValueCharacter.select(this, attribute.getName()),
	...
	public void createTypeExternal(String typeName, String language, String source, References references) {
		...
		(new ForeignCompilerJava(this, verboseExternalOperatorTypeGeneration)).compileForeignType(typeName, language,
				source);
	...
	public abstract class OperatorAssociatedWithType {
		public OperatorAssociatedWithType(String createdByType, String owner, OperatorSignature signature,
	...
	public TypeAlpha findMST(TypeAlpha udt, ValueAlpha value) {
		while (true)
			try {
				...
				System.out.println(
						"Generator: Attempting to recover from concurrent modification exception in findMST() caused by type(s) being loaded.");
			}
	}
	...
	private void createTypeOperators(final TypeAlpha udt, final String owner) {
		...
		if (udt.getPossrepCount() == 0) {
			if (udt.isSubtype() && udt.getSupertype().isBuiltin()) {
				// create selector per Database Explorations Chapter 21 page 348 "Selectors for
				...
				new OperatorAssociatedWithType(typeName, owner, selectorSignature, selectorReferenceToType) {
					public Value evaluate(Value[] arguments) {
						...
						if (!((ValueAlpha) arguments[0]).getTypeName().equals(typeName))
							throw new ExceptionSemantic(
									"RS0034: Selector failed.  Argument is not of type '" + typeName + "'.");
						return arguments[0];
					}
				};
			}
				...
				for (int componentAt = 0; componentAt < possrep.getComponentCount(); componentAt++) {
					...
					new OperatorAssociatedWithType(typeName, owner, theSignature, theReferenceToType) {
						public Value evaluate(Value[] arguments) {
							...
							if (value == null)
								throw new ExceptionSemantic(
										"RS0036: The value for '" + theOperatorName + "' is undefined.");
							return value;
						}
					};
				}
					...
					new OperatorAssociatedWithType(typeName, owner, selectorSignature, selectorReferenceToType) {
						public Value evaluate(Value[] arguments) {
							...
							if (!possrep.checkConstraint(Generator.this, value, database))
								throw new ExceptionSemantic("RS0037: Selector " + selectorSignature.toRelLookupString()
										+ " violates POSSREP constraint in type '" + typeName + "'.");
							possrep.runInitialiser(Generator.this, value, database);
							if (isSbyC(udt)) {
								if (!udt.checkSpecialisationConstraint(Generator.this, value, database))
									throw new ExceptionSemantic("RS0038: Selector "
											+ selectorSignature.toRelLookupString()
											+ " violates specialisation constraint in type '" + typeName + "'.");
								value.setMST(findMST(udt, value));
							}
							return value;
						}
		...
		new OperatorAssociatedWithType(typeName, owner, isSignature, isReferenceToType) {
			public Value evaluate(Value[] arguments) {
				if (isSbyC(udt)) {
					return ValueBoolean.select(Generator.this,
							((ValueAlpha) arguments[0]).getTypeName().equals(typeName));
				} else {
		...
		new OperatorAssociatedWithType(typeName, owner, treatSignature, treatReferenceToType) {
			public Value evaluate(Value[] arguments) {
				if (isSbyC(udt)) {
					if (!((ValueAlpha) arguments[0]).getTypeName().equals(typeName))
						throw new ExceptionSemantic(
								"RS0039: " + treatName + " failed.  Argument is not of type '" + typeName + "'.");
					return arguments[0];
				} else {
					Type type = ((ValueAlpha) arguments[0]).getType(getDatabase());
					if (!udt.canAccept(type))
						throw new ExceptionSemantic(
								"RS0396: " + treatName + " failed.  Argument is not of type '" + typeName + "'.");
					return arguments[0];
				}
			}
		};
	...
	public Type createTypeInternal(final TypeAlpha udt, String source, References references) {
		...
		if (udt.getPossrepCount() == 0 && !udt.isUnion() && !(udt.isSubtype() && udt.getSupertype().isBuiltin()))
			throw new ExceptionSemantic(
					"RS0040: A non-UNION type must define at least one POSSREP or be an immediate subtype of a built-in type.");
	...
	public RelvarHeading alterVarRealRename(String varname, RelvarHeading relvarHeading, String oldAttributeName,
	...
	public RelvarHeading alterVarRealChangeType(String varname, RelvarHeading relvarHeading, String attributeName,
			Type newType) {
		checkRelvarIsGlobalPersistent(varname);
	...
	// Define new slots in the given operator definition to expose individual
	// possrep components (where the ValueUserdefined is assumed to be
	// a parameter in the current operation definition, with a name specified by
	// sourceValueParameterName).
				...
				currentOperatorDefinition.defineSlot(component.getName(),
						new SlotScoped(depth, component.getComponentIndex(), component.getType()) {
							public void compileGet(Generator generator) {
								...
								generator.compileInstruction(
										new OpPossrepGetComponent(component.getName(), getOffset()));
							}

							public void compileSet(Generator generator) {
								throw new ExceptionFatal(
										"RS0294: compileSet invoked on SlotScoped in PossrepComponentExposure.");
							}

							public void compileInitialise(Generator generator) {
								throw new ExceptionFatal(
										"RS0295: compileInitialise invoked on SlotScoped in PossrepComponentExposure.");
							}
						});
			}
		}
	...
	public class PossrepConstraint {
		...
		public PossrepConstraint(Possrep possrep) {
			...
			setPersistentOnlyOn();
			// Define anonymous operator of the form Fn(ValueUserdefined v) RETURNS BOOLEAN;
			// RETURN <boolexpr>; END;
	...
	public class SpecialisationConstraint {
		...
		public SpecialisationConstraint(TypeAlpha udt) {
			...
			this.udt = udt;
			setPersistentOnlyOn();
			// Define anonymous operator of the form Fn(ValueUserdefined SUPERTYPE_NAME)
			// RETURNS BOOLEAN; RETURN <boolexpr>; END;
	...
	public class PossrepInitialisation {
		...
		// Define new slots in the given operator definition to expose all of a type's
		// possrep components
		// (where the ValueUserdefined is assumed to be a parameter in the current
		...
		private class PossrepComponentExposureForInitialisation {
			...
			PossrepComponentExposureForInitialisation(final TypeAlpha udt, final String sourceValueParameterName,
					final Possrep possrepToInitialise) {
				for (int possrepIndex = 0; possrepIndex < udt.getPossrepCount(); possrepIndex++) {
					...
					for (int componentIndex = 0; componentIndex < possrep.getComponentCount(); componentIndex++) {
						...
						currentOperatorDefinition.defineSlot(component.getName(),
								new SlotScoped(depth, component.getComponentIndex(), component.getType()) {
									public void compileGet(Generator generator) {
										if (componentsRequiringInitialisation.contains(componentName)
												&& possrep != possrepToInitialise)
											throw new ExceptionSemantic("RS0044: Component '" + componentName
													+ "' has been referenced before being initialised.");
										generator.compileGet(sourceValueParameterName);
										generator.compileInstruction(
												new OpPossrepGetComponent(componentName, getOffset()));
									}

									public void compileSet(Generator generator) {
										if (possrep == possrepToInitialise)
											throw new ExceptionSemantic("RS0045: Component '" + componentName
													+ "' cannot be assigned because it is set via the selector.");
										generator.compileGet(sourceValueParameterName);
									...
									public void compileInitialise(Generator generator) {
										throw new ExceptionFatal(
												"RS0296: compileInitialise invoked on SlotScoped in PossrepComponentExposureForInitialisation.");
									}
								});
					}
				}
	...
	public class ExternalOperator {
		...
		public void endExternalOperator(String externalLanguage, String sourcecode) {
			...
			operator = (new ForeignCompilerJava(Generator.this, verboseExternalOperatorTypeGeneration))
					.compileForeignOperator(getCurrentOperatorDefinition().getSignature(), externalLanguage,
							sourcecode);
			endOperator();
			// Need to remove and redefine, because operator was initially an
			// OperatorDefinitionRel but is now an OperatorDefinitionNative derivative.
	...
	public HashSet<OperatorSignature> getPossibleTargetSignatures(OperatorDefinition startingOperator,
			OperatorSignature invocationSignature) {
	...
	public Type compileEvaluate(OperatorSignature signature) {
		...
		if (invocation.useDynamicDispatch()) {
			...
			HashSet<OperatorSignature> possibleInvocations = getPossibleTargetSignatures(currentOperatorDefinition,
					signature);
			for (OperatorSignature searchFor : possibleInvocations) {
				...
				if (operator != null) {
					...
					if (returnType != null && lastFoundReturnType != null && !lastFoundReturnType.canAccept(returnType))
						throw new ExceptionSemantic(
								"RS0054: Operator " + searchFor + " is a possible invocation target but returns type "
										+ returnType + " which is incompatible with " + lastFoundReturnType);
					if (returnType != null)
			...
			if (persistentOnly) {
				if (slot instanceof SlotScoped) {
					if (((SlotScoped) slot).getDepth() <= interactiveOperatorNestingDepth)
						throw new ExceptionSemantic("RS0057: In this context, transient global variables like "
								+ refname + " may not be referenced.");
				}
			}
		}
		return slot;
	...
	// a parameter in the current operation definition, with a name specified by
	// sourceTupleParameterName), and allow them to be set.
			...
			for (final Attribute attribute : heading.getAttributes()) {
				...
				currentOperatorDefinition.defineSlot(attribute.getName(),
						new SlotScoped(depth, index++, attribute.getType()) {
							...
							public void compileSet(Generator generator) {
								TupleAttributeExposure.this.compileSet(generator, attribute, getDepth(), depth,
										getOffset());
							}

							public void compileInitialise(Generator generator) {
								throw new ExceptionFatal(
										"RS0297: compileInitialise invoked on SlotScoped in TupleAttributeExposure.");
							}
						});
			}
		}

		abstract void compileSet(Generator generator, Attribute attribute, int slotDepth, int operatorDepth,
				int offset);
	}
	...
	private abstract class AnonymousTupleOperator {
		...
		AnonymousTupleOperator(TypeTuple sourceType, Type returnType) {
			...
			// Subsequent dereference operations on original tuple attributes will use the
			// following:
			new TupleAttributeExposure(getTupleParameterName(), sourceType.getHeading()) {
				public void compileSet(Generator generator, Attribute attribute, int slotDepth, int operatorDepth,
						int offset) {
		...
		public abstract void compileSet(Generator generator, Attribute attribute, int slotDepth, int operatorDepth,
				int offset);
	}
	...
	private class TupleFilterOperator extends AnonymousTupleOperator {
		...
		public void compileSet(Generator generator, Attribute attribute, int slotDepth, int operatorDepth, int offset) {
			throw new ExceptionFatal(
					"RS0298: Attempt to set attribute " + attribute.getName() + " in " + contextDescription + ".");
		}
	}
	...
	private class TupleMapOperator extends AnonymousTupleOperator {
		...
		public void compileSet(Generator generator, Attribute attribute, int slotDepth, int operatorDepth, int offset) {
			if (slotDepth != operatorDepth)
				throw new ExceptionSemantic("RS0058: " + contextDescription
						+ " may only assign values to the most local tuple's attributes.");
	...
	public class DeleteHandler {
		 ...
		 *            - name of relvar
		 * @param operand
		 *            - TypeRelation of relation operand
		 * @return - DeleteHandler
		 */
		public DeleteHandler(String relvarName, TypeRelation operand) {
			reference = findReference(relvarName);
			if (!(reference instanceof Relvar || reference instanceof RelvarPrivate
					|| reference.getType().toString().startsWith("RELATION")))
				throw new ExceptionSemantic(
		...
		public TypeRelation endDeleteHandler(Type expressionType) {
			if (whereOperator != null) {
				...
				if (!operandType.canAccept(expressionType))
					throw new ExceptionSemantic(
							"RS0060: Expected expression of type " + operandType + " but got " + expressionType);
	...
	public class UpdateWhere {
		...
		/**
		 * To be used immediately prior to a relvar update's optional WHERE clause
		 * expression.
		 */
		...
		/**
		 * To be used immediately after a relvar update's optional WHERE clause
		 * expression.
		 */
		...
		public void beginRelvarUpdateAssignment() {
			// Define anonymous function that accepts a tuple as an argument and returns a
			// tuple
	...
	/**
	 * Assuming a Value on the stack of Type sourceType is to be assigned to a slot
	 * of Type destinationType, project the Value to conform to destinationType.
	 * This should only compile a projection if the source and destination are both
	 * of TypeHeading, or are both of TypeArray.
	 * 
	...
	public Type compileReformat(Type destinationType, Type sourceType) {
		...
		if (destinationType instanceof TypeArray && sourceType instanceof TypeArray) {
			...
			if (!destinationType.requiresReformatOf(sourceType))
				return destinationType;
			AttributeMap reformatMap = new AttributeMap(((TypeHeading) destinationElementType).getHeading(),
					((TypeHeading) sourceElementType).getHeading());
			compileInstruction(new OpArrayProject(reformatMap));
		} else {
			AttributeMap reformatMap = new AttributeMap(((TypeHeading) destinationType).getHeading(),
					((TypeHeading) sourceType).getHeading());
			if (destinationType instanceof TypeTuple && sourceType instanceof TypeTuple) {
				...
				throw new ExceptionFatal("RS0299: compileReformat doesn't know how to deal with a " + destinationType
						+ " and a " + sourceType);
		}
		return destinationType;
	}
	...
	private void compileLoadArrayFromRelation(String identifier, Slot slot, TypeArray expressionType) {
		...
		if (!(targetContainedType instanceof TypeTuple))
			throw new ExceptionSemantic("RS0062: 'LOAD " + identifier
					+ "' should reference an ARRAY of TUPLEs, but references an ARRAY of " + targetContainedType);
		Type sourceContainedType = expressionType.getElementType();
		if (!targetContainedType.canAccept(sourceContainedType))
			throw new ExceptionSemantic("RS0063: 'LOAD " + identifier + "' expected an ARRAY of " + targetContainedType
					+ " but got an ARRAY of " + sourceContainedType);
	...
	private void compileLoadRelationFromArray(String identifier, Slot slot, TypeArray expressionType) {
		...
		if (!(containedType instanceof TypeTuple))
			throw new ExceptionSemantic("RS0064: 'LOAD " + identifier
					+ "' should reference an ARRAY of TUPLEs, but references an ARRAY of " + containedType);
		...
		if (!target.canAccept(source))
			throw new ExceptionSemantic("RS0065: 'LOAD " + identifier + "' expected an ARRAY with TUPLEs of heading "
					+ targetHeading + " but got " + sourceHeading);
	...
	public void compileLoad(String identifier, Type expressionType) {
		...
		if (slot.getType() instanceof TypeRelation) {
			if (expressionType instanceof TypeArray)
				...
				throw new ExceptionSemantic("RS0066: 'LOAD " + identifier
						+ " FROM ...' expected an expression of type ARRAY, but got " + expressionType);
		} else if (slot.getType() instanceof TypeArray) {
			if (expressionType instanceof TypeArray)
				compileLoadArrayFromRelation(identifier, slot, (TypeArray) expressionType);
			else
				throw new ExceptionSemantic("RS0067: 'LOAD " + identifier
						+ " FROM ...' expected an expression of type ARRAY (usually via ORDER), but got "
						+ expressionType);
		} else
			throw new ExceptionSemantic("RS0068: 'LOAD " + identifier
					+ "', should reference a RELATION-valued variable or an ARRAY variable, but references a variable of type "
					+ slot.getType());
	}

	/**
	 * Small DIVIDEBY A1 - attributes common to r1 and r3 A2 - attributes common to
	 * r2 and r3
	 * 
	...
	/**
	 * Great DIVIDEBY A1 - attributes common to r1, r3 A2 - attributes common to r2,
	 * r4 A3 - attributes common to r3, r4
	 * 
	 * WITH r1 {A1} as r1p, r4 {A2, A3} as r4p: (r1p JOIN r2 {A2}) MINUS ((r1p JOIN
	 * r4p) MINUS (r3 {A1, A3} JOIN r4p)) {A1, A2}
	 **/
	...
	public class Extend {
		 ...
		 * 
		 * This is to be used immediately before any extend expressions, to introduce
		 * the existing tuple's attributes into the current scope so they can be
		 * dereferenced, and to introduce the extend attributes so they can be assigned.
		 */
		public Extend(Heading sourceHeading) {
			...
			// Define anonymous function that accepts two tuples as arguments and returns a
			// tuple
			...
			new TupleAttributeExposure(sourceTupleParmName, sourceHeading) {
				public void compileSet(Generator generator, Attribute attribute, int slotDepth, int operatorDepth,
						int offset) {
					throw new ExceptionFatal(
							"RS0300: Attempt to set original attribute '" + attribute.getName() + "' in EXTEND.");
				}
			};
		}

		/**
		 * This should be called immediately after the compilation of an expression
		 * whose result will be assigned to the given new identifier.
		 */
		public void addExtendItem(String identifier, Type expressionType) {
			...
			SlotScoped newSlot = new SlotScoped(depth, extendedHeading.getDegree(), expressionType) {
				...
				public void compileSet(Generator generator) {
					if (getDepth() != depth)
						throw new ExceptionSemantic(
								"RS0069: EXTEND may only assign values to newly-defined attributes.");
					generator.compileGet(extendTupleParmName);
		...
		/**
		 * End extend definition. This is invoked immediately after any extend
		 * expressions.
		 */
	...
	public Type compileTClose(Type expressionType) {
		...
		if (exprHeading.getDegree() != 2)
			throw new ExceptionSemantic(
					"RS0071: TCLOSE expected RELATION of degree 2, but degree is " + exprHeading.getDegree());
		if (!exprHeading.getAttributes().get(0).getType().getSignature()
				.equals(exprHeading.getAttributes().get(1).getType().getSignature()))
			throw new ExceptionSemantic(
					"RS0072: TCLOSE expected both attributes of the RELATION to be the same type, but they aren't.");
	...
	public TypeRelation compileRelationTimes(TypeRelation leftType, TypeRelation rightType) {
		...
		if (intersect.getDegree() == 0)
			...
			throw new ExceptionSemantic(
					"RS0074: Attempt to perform TIMES on operands with attributes in common.  Perhaps you want to use JOIN or RENAME?");
		return new TypeRelation(result);
	}
	...
	public TypeRelation compileRelationGroup(TypeRelation sourceType, SelectAttributes itemList, String name) {
		...
		compileInstruction(new OpRelationGroup(new AttributeMap(sortAttributeHeading, sourceHeading),
				new AttributeMap(groupedAttributeHeading, sourceHeading)));
	...
	public TypeRelation compileRelationUngroup(TypeRelation sourceType, String attributeName) {
		...
		if (!(rvaType instanceof TypeRelation))
			throw new ExceptionSemantic(
					"RS0076: Expected attribute '" + attributeName + "' to be a relation, but got " + rvaType);
		Heading rvaHeading = ((TypeRelation) rvaType).getHeading();
		if (sourceHeading.intersect(rvaHeading).getDegree() > 0)
			throw new ExceptionSemantic(
					"RS0077: Relation-valued attribute '" + attributeName + "' shares attributes with " + sourceType);
	...
	public class RelationSubstitute {
		 ...
		 * 
		 * This is to be used immediately before an assignment statement, to introduce a
		 * tuple's attributes into the current scope so they can be dereferenced and
		 * assigned.
		 * 
		 * @param sourceType
		 */
	...
	public class TupleSubstitute {
		 ...
		 * assigned.
		 * 
		 * The ValueTuple on the stack will be subject to a non-destructive update via
		 * the assignment statement. A new ValueTuple of the same TypeTuple will be
	...
	public TypeTuple compileTupleUnwrap(TypeTuple sourceType, String attributeName) {
		...
		if (!(tupleAttribute.getType() instanceof TypeTuple))
			throw new ExceptionSemantic("RS0080: Expected attribute '" + tupleAttribute.getName()
					+ "' to be a TUPLE but it is a " + tupleAttribute.getType());
	...
	public TypeTuple compileTupleWrap(TypeTuple sourceType, SelectAttributes selection, String name) {
		// Create a tuple type consisting of the attributes being wrapped by new
		// attribute
	...
	public TypeTuple compileTupleDUnion(TypeTuple leftType, TypeTuple rightType) {
		...
		if (left.intersect(right).getDegree() == 0) {
			...
			throw new ExceptionSemantic(
					"RS0081: Attempt to perform disjoint union on tuple types that have attributes in common.");
	}
	...
	public String getOperatorDefinitionLineReferenceStack(int lineNumber) {
		...
		while (true) {
			if (!current.isSpecial())
				out.append("\tIn " + current.getSignature().toString() + " line "
						+ (lineNumber - current.getStartLine() + 1));
	...
	public void compileReturnValue(Type returnType) {
		if (!currentOperatorDefinition.hasReturnDeclaration())
			throw new ExceptionSemantic("RS0083: Operator " + currentOperatorDefinition.getSignature()
					+ " has not declared a return type but has defined a return expression.");
	...
	public void compileReturn() {
		if (currentOperatorDefinition.hasReturnDeclaration())
			throw new ExceptionSemantic("RS0084: Operator " + currentOperatorDefinition.getSignature()
					+ " has declared a return type, but the RETURN statement is missing an expression of that type.");
	...
	public void endOperator() {
		if (currentOperatorDefinition.hasReturnDeclaration()) {
			if (!currentOperatorDefinition.hasDefinedReturnValue())
				throw new ExceptionSemantic("RS0085: Operator " + currentOperatorDefinition.getSignature()
						+ " has declared a return type, but contains no RETURN statement of that type.");
	...
	/**
	 * Return true if current operator definition is top level, and should therefore
	 * be persistent.
	 */
	...
	public void dropOperator(OperatorSignature signature) {
		...
		if (operatorGenerationTypeName != null && operatorGenerationTypeName.length() > 0)
			throw new ExceptionSemantic("RS0088: OPERATOR " + signature + " was generated by TYPE "
					+ operatorGenerationTypeName + " and may not be dropped directly.");
		beginAssignment();
	...
	private class TupleProcessOperator extends AnonymousTupleOperator {
		...
		public void compileSet(Generator generator, Attribute attribute, int slotDepth, int operatorDepth, int offset) {
			throw new ExceptionFatal(
					"RS0302: Attempt to set attribute '" + attribute.getName() + "' in " + contextDescription + ".");
		}
	}
	...
	public Type compileEQ(Type leftType, Type rightType) {
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.EQUALS, leftType, rightType,
				TypeBoolean.getInstance());
	}

	// !=
	public Type compileNEQ(Type leftType, Type rightType) {
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.NOTEQUALS, leftType, rightType,
				TypeBoolean.getInstance());
	}
	...
	public Type compileGTE(Type leftType, Type rightType) {
		...
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.GREATERTHANOREQUALS, leftType, rightType,
				TypeBoolean.getInstance());
	}
	...
	public Type compileLTE(Type leftType, Type rightType) {
		checkNotSupportedByTuple(leftType, rightType);
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.LESSTHANOREQUALS, leftType, rightType,
				TypeBoolean.getInstance());
	}
	...
	public Type compileGT(Type leftType, Type rightType) {
		checkNotSupportedByTuple(leftType, rightType);
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.GREATERTHAN, leftType, rightType,
				TypeBoolean.getInstance());
	}
	...
	public Type compileLT(Type leftType, Type rightType) {
		checkNotSupportedByTuple(leftType, rightType);
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.LESSTHAN, leftType, rightType,
				TypeBoolean.getInstance());
	}

	public Object compileSubset(Type leftType, Type rightType) {
		checkNotSupportedByTuple(leftType, rightType);
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.SUBSET, leftType, rightType,
				TypeBoolean.getInstance());
	}

	public Object compileSubsetOrEqual(Type leftType, Type rightType) {
		checkNotSupportedByTuple(leftType, rightType);
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.SUBSETOREQUAL, leftType, rightType,
				TypeBoolean.getInstance());
	}

	public Object compileSuperset(Type leftType, Type rightType) {
		checkNotSupportedByTuple(leftType, rightType);
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.SUPERSET, leftType, rightType,
				TypeBoolean.getInstance());
	}

	public Object compileSupersetOrEqual(Type leftType, Type rightType) {
		checkNotSupportedByTuple(leftType, rightType);
		return compileComparisonOperatorInvocation(BuiltinTypeBuilder.SUPERSETOREQUAL, leftType, rightType,
				TypeBoolean.getInstance());
	}
	...
	public class RelationDefinition {
		...
		// Begin relation literal. If heading is null, first tuple added will define the
		// type.
		...
		public void addTupleToRelation(TypeTuple tupleType) {
			...
			else if (!heading.canAccept(tupleHeading))
	...
	public class TupleDefinition {
			...
			if (wildcard) {
				...
				while (currentOperator != null) {
					...
					for (int parm = 0; parm < parmCount; parm++) {
						...
						// Are we in an open expression context? If so, a parm "%tuple<n>" or
						// "%source_tuple<n>" will exist.
						// Let TUP {*} refer to it.
		...
		// Identify this as a "wildcard" tuple, i.e., TUPLE {*}, which should close over
		// its scope
	...
	public void compilePushDelimitedString(String value) {
		...
		if (start != end)
			throw new ExceptionSemantic(
					"RS0503: A CHARACTER literal must start and end with the same delimiter, not start with " + start
							+ " and end with " + end + ".");
		if (start != '\'' && start != '"')
			throw new ExceptionSemantic(
					"RS0504: A CHARACTER literal must start and end with ' or \". Unicode asymmetric quotes are not allowed.");
