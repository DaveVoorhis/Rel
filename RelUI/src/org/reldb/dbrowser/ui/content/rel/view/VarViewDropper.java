	...
	public void go(DbTreeItem item) {
		...
		if (tab != null) {
			...
			if (!MessageDialog.openConfirm(relPanel.getShell(), "Confirm DROP", "Are you sure you wish to drop var " + item.getName() + "?"))
				return;
			if (!relPanel.getConnection().execute("DROP VAR " + item.getName() + ";"))
				MessageDialog.openError(relPanel.getShell(), "Error", "Unable to drop var " + item.getName() + ". Check the system log for details.");
			else
