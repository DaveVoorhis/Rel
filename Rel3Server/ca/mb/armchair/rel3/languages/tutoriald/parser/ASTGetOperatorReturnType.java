/* Generated By:JJTree: Do not edit this line. ASTGetOperatorReturnType.java Version 4.3 */
/* JavaCCOptions:MULTI=true,NODE_USES_PARSER=false,VISITOR=true,TRACK_TOKENS=false,NODE_PREFIX=AST,NODE_EXTENDS=ca.mb.armchair.rel3.languages.tutoriald.BaseASTNode,NODE_FACTORY=,SUPPORT_CLASS_VISIBILITY_PUBLIC=true */
package ca.mb.armchair.rel3.languages.tutoriald.parser;

public
class ASTGetOperatorReturnType extends SimpleNode {
  public ASTGetOperatorReturnType(int id) {
    super(id);
  }

  public ASTGetOperatorReturnType(TutorialD p, int id) {
    super(p, id);
  }


  /** Accept the visitor. **/
  public Object jjtAccept(TutorialDVisitor visitor, Object data) {
    return visitor.visit(this, data);
  }
}
/* JavaCC - OriginalChecksum=9df8e36b3d6cfe5a0252848cb2f71755 (do not edit this line) */
