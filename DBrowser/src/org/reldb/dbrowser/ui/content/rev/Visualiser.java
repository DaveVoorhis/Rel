...
public abstract class Visualiser extends Composite implements Comparable<Visualiser> {
	...
	protected Visualiser(Model model, String id, String title, int xpos, int ypos) {
		...
		btnInfo.setText("?");
		btnInfo.addListener(SWT.Selection, e -> (new ViewQueryDialog(Visualiser.this)).open());
		...
		btnEdit.setText("+");
		btnEdit.addListener(SWT.Selection, e -> model.getRev().showEditorFor(title));
		...
		if (!model.getRev().isReadOnly()) {
			lblTitle.addMouseListener(new MouseAdapter() {
				...
				public void mouseUp(MouseEvent e) {
					...
					if (dropCandidate != null) {
						...
						if (dropCandidate == model.getPossibleDropTarget(e.x + location.x, e.y + location.y,
								Visualiser.this)) {
			...
			lblTitle.addMouseMoveListener(e -> {
				...
				if (dragging) {
					...
					// drop?
					Visualiser dropTarget = model.getPossibleDropTarget(location.x + e.x, location.y + e.y,
							Visualiser.this);
	...
	/**
	 * Invoked when the model changes, to allow the visualiser to update itself
	 * accordingly.
	 */
	public void verify() {
		for (Argument argument : arguments)
