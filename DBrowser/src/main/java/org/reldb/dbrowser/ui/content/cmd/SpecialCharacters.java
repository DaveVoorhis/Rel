...
public class SpecialCharacters extends Dialog {
	...
	private static class SpecialCharacter {
		...
		public boolean matches(KeyEvent e) {
			...
			if ((e.stateMask & SWT.SHIFT) != 0)
				return (Character.isUpperCase(altOf) && e.keyCode == Character.toLowerCase(altOf))
						|| e.keyCode == actualKeypress;
			return e.keyCode == altOf;
		}
	}

	private static SpecialCharacter[] specialCharacters = { new SpecialCharacter('\u00D7', 't', "Times"),
			new SpecialCharacter('\u00F7', 'd', "Divide"), new SpecialCharacter('\u203C', '!', "Image in", '1'),
			new SpecialCharacter('\u2208', 'e', "IN (is element of)"), new SpecialCharacter('\u2260', '=', "Not equal"),
	...
	private void createContents() {
		...
		for (SpecialCharacter special : specialCharacters) {
			...
			charButton.addListener(SWT.Selection, e -> emit(special.symbol));
